...
class CacheActivationsRunner:
    def __init__(
        ...
        if self.cfg.hook_names is not None:
            ...
            self.model.unet.to(self.accelerator.device)
            ...
            self.scheduler_timesteps = self.model.scheduler.timesteps
            ...
            self.num_examples = self.cfg.max_num_examples
            # self.dataloader = self.get_batches(self.dataset, self.cfg.batch_size)
            self.dataloader = self.dataset.iter(self.cfg.batch_size)
            self.n_buffers = self.cfg.max_num_examples // self.cfg.batch_size
        ...
        for i, batch in tqdm(
            ...
            with self.accelerator.split_between_processes(batch) as prompt:
                ...
                _, acts_cache = self.model.run_with_cache(
                    ...
                    num_frames=30,  # TODO move to config
